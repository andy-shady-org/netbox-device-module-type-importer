...
from netbox_device_type_importer.tables import MetaModuleTypeTable
from netbox_device_type_importer.filtersets import MetaModuleTypeFilterSet
from netbox_device_type_importer.forms import MetaModuleTypeFilterForm
from netbox_device_type_importer.utilities import GitHubGQLAPIAsync, GQLError

__all__ = (
    "MetaModuleTypeListView",
...
class MetaModuleTypeListView(generic.ObjectListView):
    ...
    table = MetaModuleTypeTable
    action_buttons = ("delete",)
    actions = (BulkDelete,)
    template_name = "netbox_device_type_importer/metamoduletype_list.html"
...
@register_model_view(MetaModuleType, "bulk_delete", path="delete", detail=False)
...
class MetaModuleTypeEditView(ContentTypePermissionRequiredMixin, View):
    ...
    def post(self, request):
        ...
        created = 0
        updated = 0
        if not request.user.has_perm("netbox_device_type_importer.add_metamoduletype"):
            return HttpResponseForbidden()
        plugin_settings = settings.PLUGINS_CONFIG.get("netbox_device_type_importer", {})
        ...
        if token:
            gh_api = GitHubGQLAPIAsync(
                url=url, token=token, owner=owner, repo=repo, path="module-types"
            )
        else:
            messages.error(request, message=f"No Github Token Found")
            return redirect("plugins:netbox_device_type_importer:metamoduletype_list")
        try:
        ...
        existing_records = {
            ...
            for record in MetaModuleType.objects.all()
        }
...
class MetaModuleTypeImportView(ContentTypePermissionRequiredMixin, View):
    ...
    def get_required_permission(self):
        return "netbox_device_type_importer.add_metamoduletype"
    ...
    def post(self, request):
        ...
        for sha, yaml_text in dt_files.items():
            ...
            if form.is_valid():
                ...
                data["_init_time"] = 1
                model_form = forms.ModuleTypeImportForm(data)
                # is it necessary?
                ...
                if model_form.is_valid():
                    try:
                        ...
                        messages.error(
                            request,
                            f"Failed to import Module Type, exception raised: {str(e)}",
                        )
                        pass
                ...
                if model_form.errors:
                    ...
                    messages.error(
                        request,
                        f"Failed to import Module Type, form has errors: {model_form.errors}",
                    )
                else:
                    imported_dt.append(obj.pk)
                    metadt = MetaModuleType.objects.get(sha=sha)
                    metadt.imported_dt = obj.pk
                    metadt.save()
            else:
                errored += 1
                messages.error(
                    request, f"Failed to import Module Type, form not valid: {obj.name}"
                )
        # msg
        if imported_dt:
            ...
            return redirect(reverse("dcim:moduletype_list") + "?" + qparams)
        else:
            messages.error(request, f"Can not import Module Types")
            return redirect("plugins:netbox_device_type_importer:metamoduletype_list")
